...
define ["raphael", "jquery", "underscore", "Chain", "Board", "domReady!", ], (Raphael, $, _, Chain, Board) ->
  ...
  # text labels
  _.each _.range(n - 1), (letter, index) ->
  ...
  _.each _.range(1, n), (letter, index) ->
  ...
  white_stone = (i, j) ->
    _x = x + cell_radius * i
    _y = y + cell_radius * j
    circle2 = paper.circle(_x, _y, circle_radius)
    circle2.attr "fill", "#fff"
    circle2.attr "stroke-width", "0"
    circle = paper.circle(_x, _y, circle_radius)
  ...
  group.mouseover((e) ->
    ...
    switch move_results.color
      when virtual_board.BLACK
        black_stone(move_results.x, move_results.y)
        track_stone(move_results.x, move_results.y)
      when virtual_board.WHITE
        white_stone(move_results.x, move_results.y)
        track_stone(move_results.x, move_results.y)
      else
        # do nothing
    return
